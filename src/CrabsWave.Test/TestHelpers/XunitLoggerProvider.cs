    ...
    public class XunitLogger : ILogger
    {
            ...
            LogLevel logLevel, EventId eventId, TState state, Exception exception, Func<TState, Exception, string> formatter)
        {
            ...
            catch (Exception)
            {
                ...
                // ITestOutputHelper is busted (if the test "completed" before the background thread
                // fired). So, ignore this. There isn't really anything we can do but hope the caller
                // has additional loggers registered
            }
        }
